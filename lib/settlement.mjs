...
tax proration to 1100; an escrow holdback to 1510. **Anything an HOA charges to complete the sale -
a resale certificate, a transfer fee, a release, and a reimbursement of one - is 1340, not 1130 and
not 1010:** it is bought in order to sell, so it is a selling cost (Paul, 2026-09-22). Ordinary HOA
dues for a period while the house was held are 1130. A
...
export function validateSettlement(raw) {
  ...
  const lines = [];
  // A statement often lists a charge with no amount in the seller-paid column (an HOA fee
  // the buyer pays, a premium disclosed for information). Dropping it is right and it is
  // not something to check, so it is noted rather than flagged (2026-09-22).
  const zeroLines = [];
  for (const [i, line] of (Array.isArray(r.lines) ? r.lines : []).entries()) {
    ...
    if (cents === 0) { zeroLines.push(String(line?.label || "").trim() || `account ${line?.account || ""}`); continue; }
