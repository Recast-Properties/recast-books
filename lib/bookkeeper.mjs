...
export const MAX_TOKENS_PER_TURN = 16000; // adaptive thinking counts against this; 8000 cut two reads dead mid-thought (2026-09-23/25). Above ~21k the SDK refuses a non-streaming call ("Streaming is required ... 10 minutes"), which errored five reads on 2026-09-25.
...
const DECIDE_ENTRY_SCHEMA = {
  ...
  properties: {
    ...
    paid_from: { type: "string", description: "A 14xx bank account code, \"PAUL\", \"DENNIS\", or \"UNKNOWN\" when nothing on the document identifies the payer" },
...
const DECIDE_SCHEMA = {
  ...
  additionalProperties: false,
  required: [
    "verdict", "confidence", "why", "checked", "document_type", "vendor", "date",
  ...
  properties: {
    ...
    why: { type: "string", description: "ONE sentence, under 20 words, for Paul's digest: the decision and its single reason. No narrative." },
    checked: { type: "string", description: "The verification record for the reviewer: which fields of the document were zoomed, how the arithmetic reconciled, which ledger and document lookups were made, and what stayed unresolved. Shown only when a reviewer opens the item." },
    ...
    paid_from: { type: ["string", "null"], description: "A 14xx bank account code, \"PAUL\", \"DENNIS\", or \"UNKNOWN\" if nothing on the document identifies the payer" },
    paid_from_reason: { type: "string", description: "What you saw, or what was missing when UNKNOWN" },
...
function buildTools() {
  const clientTools = [
    ...
    },
    {
      ...
      description:
        "The properties currently held (name, address, purchase date), plus the literal value OVERHEAD. This is the only valid set of `property` values - never invent one.",
      strict: true,
...
/**
 * The image type the bytes actually are, or "" when not recognised. The API rejects
 * an image whose declared media_type disagrees with its bytes (2026-09-18: a JPEG
 * thumbnail forwarded as image/png, gm-19c521cfa5452bd9), so the bytes win.
 */
export function sniffImageMime(buffer) {
  if (!buffer || buffer.length < 12) return "";
  if (buffer[0] === 0xff && buffer[1] === 0xd8 && buffer[2] === 0xff) return "image/jpeg";
  if (buffer.readUInt32BE(0) === 0x89504e47) return "image/png";
  const ascii = buffer.toString("latin1", 0, 12);
  if (ascii.startsWith("GIF")) return "image/gif";
  if (ascii.startsWith("RIFF") && ascii.slice(8) === "WEBP") return "image/webp";
  return "";
}

// iPhone photos are often HEIC, which neither Claude nor jimp can read. Same
// conversion the old receipts ingest has used since 2026-06-26 (heic-convert);
// the Phase 2 spec wrongly assumed jimp could decode HEIC, so every HEIC under the
// poller's 3 MB Drive-shrink cap was held as "could not be converted" (2026-09-16).
...
function buildHeaderText(envelope) {
  ...
  // phase2.6-spec.md §4: the mailbox hint, right under Channel where it's most visible.
  if (ctx?.property_mailbox_hint) {
    lines.push(
      `This document was sent to the mailbox for property ${clean(ctx.property_mailbox_hint)}. ` +
        "Post it to that property unless the document itself plainly names a different one or is company overhead (then say why in `why`).",
    );
  }
  lines.push(
  ...
  if (ctx) {
    ...
    lines.push("No card shown or legible -> paid_from UNKNOWN; Paul assigns the account (D-014).");
  }
  return lines.join("\n");
}
...
async function buildUserContent(envelope, attachments) {
  ...
  for (const att of attachments || []) {
    ...
    // Belt and braces for the poller's normalization: a PDF or image labelled
    // octet-stream (or nothing) is still a PDF or image - decide by extension.
    if (!mime || mime === "application/octet-stream" || mime === "binary/octet-stream") {
      const ext = (clean(att?.name).toLowerCase().match(/\.([a-z0-9]+)$/) || [])[1] || "";
      mime = ext === "pdf" ? "application/pdf"
        : ext === "jpg" || ext === "jpeg" ? "image/jpeg"
        : /^(png|gif|webp|heic|heif)$/.test(ext) ? `image/${ext}` : mime;
    }
    ...
    // Magic bytes beat the declared type and the extension; those are the fallback.
    ...
    // D-035: a body-only email is stored as email.txt so it files to Drive; its text is
    // already the message body shown above, so there is nothing more to show.
...
function normalizeDecide(rawInput) {
  ...
  const out = {
    ...
    confidence,
    why: clean(input?.why),
    // The verification record. Asked for since 2026-09-22 (why = one sentence, the working
    // goes here) and written by the model every time - and dropped right here until
    // 2026-09-25, so the Inbox's "Claude's read" never showed it.
    checked: clean(input?.checked).slice(0, 4000),
...
export async function runBookkeeper({ envelope, attachments, deps }) {
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    let res;
    try {
      // beta.messages + fallbacks: a safety-classifier refusal (stop_reason "refusal",
      // e.g. category reasoning_extraction - the first five properties@ reads on
      // 2026-09-22, audit §56) re-runs the same request on a fallback model inside
      // the call instead of landing in Pending as a hold.
      res = await deps.anthropic.beta.messages.create({
      ...
      // Not a hold: a failed call is not a read. Thrown, it lands in ingest's catch as
      // an `error` envelope with no `model`, which the warm job retries and the Inbox
      // shows as a failure (2026-09-18: a 400 sat in Pending looking like a normal hold).
      throw new Error(`The Anthropic API call failed (turn ${turn + 1}): ${msg}`, { cause: err });
    }

    accumulateUsage(usage, res?.usage);
    if ((res?.usage?.iterations || []).some((it) => it && it.type === "fallback_message")) {
      transcript.push(`turn ${turn + 1}: served by fallback model ${res.model || "?"}`);
    }
