...
const SYSTEM_PROMPT = readFileSync(path.join(__dirname, "bookkeeper-prompt.md"), "utf8");
...
const DECIDE_ENTRY_ITEM_SCHEMA = {
  ...
  properties: {
    ...
    trade: { type: "string", description: "Trade this line belongs to (e.g. \"Paint & Flooring\"), or \"\"" },
    business_purpose: { type: "string", description: "Required (non-empty) for accounts 6600, 6700, 6710, 6720; \"\" otherwise" },
  },
};
...
const DECIDE_SCHEMA = {
  ...
  required: [
    ...
    "paid_from", "paid_from_reason", "duplicate_of", "supersedes", "entries",
  ],
  properties: {
    ...
    vendor: { type: "string" },
    date: { type: "string", description: "YYYY-MM-DD, or \"\" if unknown" },
    receipt_total_cents: { type: "integer" },
    subtotal_cents: { type: ["integer", "null"] },
    tax_cents: { type: ["integer", "null"] },
    paid_from: { type: "string", description: "A 14xx bank account code, \"PAUL\", or \"DENNIS\"; \"\" if you could not tell" },
    paid_from_reason: { type: "string", description: "What you saw, or which default you fell back to" },
    duplicate_of: { type: "string", description: "txn_id or docId this document duplicates, when verdict is dismiss" },
    supersedes: { type: "string", description: "txn_id this document is the corrected/final version of" },
...
function normalizeDecide(input) {
  ...
  return {
    ...
    duplicate_of: clean(input?.duplicate_of),
    supersedes: clean(input?.supersedes),
