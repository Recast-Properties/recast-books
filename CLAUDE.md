...
are the target, D-028 returns hold for Paul). **Resume from `HANDOFF-2026-09-22.md`, then `docs/cutover-runbook.md` and `docs/phase4-audit.md` §45-§50** - state on 2026-09-21:
...
- **Staging (2026-09-21 evening):** pass 11 tied out on both paths - 1,048 entries, $240,844.35, $0.00 on all ten properties, **922 linked, every link a Drive file** (item 7 done: `drive-filing.json`, 423 keys / 636 files, `evidence/` filed; audit §45-§49). Tie-out tool: `scripts/migration-journal-tieout.py`. Corrections now C-1 … C-33; `paul-answers.json` gained an `account` key. **Next: `docs/cutover-runbook.md`** - one rehearsal Run of `migrationRegisterAdvances` in staging, then cutover day. `WRITER_SECRET` is masked on Netlify: any local script that calls the writer is started by Paul in his Terminal.
