...
are the target, D-028 returns hold for Paul). **Resume from `HANDOFF-2026-09-21.md`, then `docs/phase4-audit.md` §39-§43** - state on 2026-09-21:
- **Numbers:** dry run **1,048 entries, $240,844.35**, ten property totals (`rows/expected.json`), all build in the posting engine; **920 linked (87.8% of rows, 78.6% of dollars)**; 128 rows / $51.4K with no document, of which $36,880 Paul accepted (contractor checks and cash) - the rest is cash labor and small rows, proven in Phase 3. 33 advances. Corrections **C-1 … C-32** in audit §13; decisions D-027 … D-034 (D-034: property tax - payments post when paid, the closing proration posts at closing, the tab estimates in between).
- **Staging:** the 1,048-entry dry run and the current writer are PUSHED to the staging project (verified by `clasp pull`, 2026-09-21) and **need one Run of `migrationRunStaging`**, then a tie-out. Staging still holds pass 9 ($240,639.05, tied out; `rows/journal-tieout.json`). `migrationRunStaging` refreshes the Properties rows itself (tax_annual, notes); `migrationRegisterAdvances` has a skip-rebuild flag, pushed and not yet run.
...
- **Facts corrected on 2026-09-18:** 104 Ashburne has NOT closed - still held (only Newport's settlement date is owed). "Effren" rows are **Falcon Creek Lawn Care** (`rename_payee`). Paul's notes are the documents for cash labor.
...
- **Open, in order:** (1) the staging Run and tie-out above; (2) **item 7 - Drive filing**: 408 documents still carry a Gmail link, 17 linked paul@ messages were never read (~$0.21 each), and `data/migration/2026-09-17/evidence/` (nine screenshots, a PDF, a text file - committed here, **no remote, this Mac holds the only copy**) must go to Drive; entries flagged `EVIDENCE:<file>` get their `doc_url` there; (3) cutover day - production writer push + `clasp deploy -i`, Newport's settlement date, `clearBooks` → register properties and advances → the same pass → tie-out → replay of the live receipts Paul has held since 09-11 → `WRITER_URL` back to production (`npm run deploy` is Paul's step) → old workbook re-exported, diffed against the 09-17 snapshot, set read-only. Owed by others: Dennis's PayPal receipts for the Granite, Sparkling and Newport listing fees. `clasp` needs a fresh `npx clasp login` every few days (Workspace re-auth).
