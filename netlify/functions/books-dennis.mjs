...
async function loadAdvances(writer) {
  ...
  return rows.map((r) => ({
    ...
    notes: r.notes || "",
    kind: r.kind || "",
    // D-022: per-advance rate, stored as a percent ("8"); undefined -> Settings rate
    rate_annual: r.rate_pct !== "" && r.rate_pct != null && Number.isFinite(Number(r.rate_pct)) ? Number(r.rate_pct) / 100 : undefined,
  }));
}
...
export default async (req) => {
  ...
  if (req.method === "POST") {
    ...
    if (body.action === "addAdvance") {
      ...
      // The property tab rebuild (its advance schedules size to the Advances rows) is
      // the client's next call, not part of this one: it is slow and this request
      // should answer as soon as the advance is on the books (Paul, 2026-09-15).
